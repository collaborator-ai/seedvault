...
The daemon exposes a Unix domain socket for local consumers to receive real-time file events. Events mirror the two mutations the daemon sends to the server: file upsert and file delete.

**Socket path:** `~/.config/seedvault/daemon.sock`
...
```json
{"action":"file_write","path":"notes/seedvault.md","collection":"notes","timestamp":"2026-02-10T22:05:00Z"}
{"action":"file_delete","path":"notes/old-idea.md","collection":"notes","timestamp":"2026-02-10T22:06:00Z"}
```

| Field | Type | Description |
|-------|------|-------------|
| `action` | `"file_write"` \| `"file_delete"` | Matches the PUT/DELETE sent to the server |
| `path` | string | Server-relative path (collection prefix + relative file path) |
| `collection` | string | Collection name the file belongs to |
| `timestamp` | string | ISO 8601 timestamp of when the event occurred |

**Programmatic access:**

The `subscribeDaemonEvents()` async generator is exported from the CLI's public API:

```typescript
import { subscribeDaemonEvents } from "seedvault/api";

for await (const event of subscribeDaemonEvents()) {
